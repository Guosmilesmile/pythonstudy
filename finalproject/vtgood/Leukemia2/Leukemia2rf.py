...
def getTargetData(fielname):
    ...
    for i in range(len(train_classfi)):
        ...
        if train_classfi[i]=="ALL" :
            klass = 0
            train_classfi_number.append(klass)
        elif train_classfi[i]=="MLL" :
            klass = 1
            train_classfi_number.append(klass)
...
def generate_data():
    ...
    train_text,train_classfi_number,train_classfi,train_feature_name = getTargetData("Leukemia2_train.data")
    X = train_text
...
def generate_Testdata(index):
    ...
    #Load boston housing dataset as an example
    train_text,train_classfi_number,train_classfi,train_feature_name = getTargetData("Leukemia2_test.data")
    temp=index
    ss = []
    count = 0
    df = pd.DataFrame(train_text)
...
def perceptron(hidden_neurons=20, weightdecay=0.01, momentum=0.1):
    ...
    df.to_excel("Leukemia2out.xls")
    df = pd.DataFrame(result)
    df.insert(1,'1',tstdata['class'])
    df.to_excel("Leukemia2.xls")
    error = 0;
    ...
    NetworkWriter.writeToFile(fnn, 'Leukemia2.xml')
